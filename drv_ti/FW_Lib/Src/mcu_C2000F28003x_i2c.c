...
static i2c_status_t mcu_i2c_init (i2c_ctx_t *ctx) {
    ...
    I2C_disableModule(ctx->hw_cfg.base);
    DEVICE_DELAY_US(10);
    I2C_enableModule(ctx->hw_cfg.base);
    return I2C_OK;
}
...
static i2c_status_t mcu_i2c_send_message (i2c_ctx_t * ctx) {
    ...
    uint16_t i = 0;
    while (i < TxLen && I2C_getTxFIFOStatus(base) < I2C_FIFO_TX16) {
        I2C_putData(base, ctx->sw_data.TxData[i++]);
    }

    // Set Controller Send Mode (TRX = 1: Write) and set START & STOP bits atomically
    HWREGH(base + I2C_O_MDR) |= (I2C_MDR_IRS | I2C_MDR_MST | I2C_MDR_TRX | I2C_MDR_STT | I2C_MDR_STP);

    //Add data to FIFO when TxLen > 16
    while (i < TxLen) {
        timeout = ctx->hw_cfg.timeout_cycles;
        while (I2C_getTxFIFOStatus(base) == I2C_FIFO_TX16) {
            if (I2C_getStatus(base) & I2C_STS_NO_ACK) goto handle_nack;
...
static i2c_status_t mcu_i2c_receive_message (i2c_ctx_t *ctx) {
    if (ctx == NULL || ctx->sw_data.RxData == NULL || ctx->sw_data.RxLen == 0) {
        return I2C_ERROR;
    }
    ...
    //Set slave target address and expected data count
    I2C_setTargetAddress(base, ctx->sw_data.TargetAddr);
    I2C_setDataCount(base, ctx->sw_data.RxLen);

    //Flush RX FIFO
    HWREGH(base + I2C_O_FFRX) &= ~I2C_FFRX_RXFFRST;
    HWREGH(base + I2C_O_FFRX) |= (I2C_FFRX_RXFFRST | I2C_FFRX_RXFFINTCLR);

    //Controller Receive Mode (TRX = 0, MST = 1, IRS = 1) + Start & Stop
    HWREGH(base + I2C_O_MDR) &= ~I2C_MDR_TRX;
    HWREGH(base + I2C_O_MDR) |= (I2C_MDR_IRS | I2C_MDR_MST | I2C_MDR_STT | I2C_MDR_STP);
